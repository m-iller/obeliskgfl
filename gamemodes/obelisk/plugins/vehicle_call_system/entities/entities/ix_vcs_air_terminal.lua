...
ENT.PrintName = "Консоль вызова воздушной техники"
ENT.Author = "kido"
ENT.Spawnable = true
ENT.AdminOnly = true
ENT.Category = "VCS"
...
if SERVER then
    ...
    function ENT:Use(activator, caller)
		...
		if (not character) or (not character:HasFlags("V")) then
			...
			local denyMsg = caller:GetName() .. " пытается использовать терминал воздушной техники, но система отказывает в доступе."
        ...
        -- RP отыгрывание
        caller:SetAction("Подключается к терминалу воздушной техники...", 3)
        
        -- Уведомляем окружающих
        local plyName = caller:GetName()
        local message = plyName .. " подключается к терминалу вызова воздушной техники."
...
if CLIENT then
    function ENT:Draw()
        ...
        cam.Start3D2D(pos, ang, 0.1)
            draw.SimpleText("КОНСОЛЬ ВЫЗОВА", "VCS_RobotoLarge", 0, -60, Color(0, 255, 0), TEXT_ALIGN_CENTER)
            draw.SimpleText("ВОЗДУШНОЙ ТЕХНИКИ", "VCS_RobotoMedium", 0, -30, Color(0, 255, 0), TEXT_ALIGN_CENTER)

...
if SERVER then
    ...
    hook.Add("CharacterLoaded", "ixImplantsApplyOnLoad", function(character)
        local client = character:GetPlayer()
        if not IsValid(client) then return end
        
        print("[Implants] Применяем эффекты имплантов при загрузке персонажа:", character:GetName())
        
        local implants = ix.GetCharacterImplants(character)
        
        -- Применяем эффекты с небольшой задержкой для корректной работы
                ...
                for limb, implant in pairs(implants) do
                    if implant and implant ~= "НЕТ" then
                        ...
                        if implantData then
                            local bonuses = implantData.bonuses or (implantData.bonus and {implantData.bonus}) or {}
                            for _, bonus in ipairs(bonuses) do
                                print("[Implants] Применяем бонус (при загрузке):", bonus, "для импланта:", implant, "на конечность:", limb)
                                
                                -- Применяем бонус напрямую
                                if bonus == "speed_boost" then
                                    client:SetWalkSpeed(client:GetWalkSpeed() * 10)
                                    client:SetRunSpeed(client:GetRunSpeed() * 10)
                                ...
                                elseif bonus == "damage_boost" then
                                    character:SetData("ixDamageMultiplier", 1.5)
                                    
                                elseif bonus == "damage_resistance" then
                                    character:SetData("ixDamageResistance", 0.6)
                                    
                                elseif bonus == "fall_damage_reduction" then
                                    character:SetData("ixFallDamageReduction", 0.3)
